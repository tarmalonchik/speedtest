...
func TestNiceMD_Set(t *testing.T) {
	nmd := metautils.NiceMD(metadata.Pairs(testPairs...))
	nmd.Set("multikey", "one")
	nmd.Set("newkey", "something")
	nmd.Set("newkey", "another")
...
func TestNiceMD_ToOutgoing(t *testing.T) {
	...
	md := metautils.ExtractOutgoing(nCtx).Clone()
	md.Set("newvalue", "something")
	eCtx := md.ToOutgoing(nCtx)
...
func TestNiceMD_ToIncoming(t *testing.T) {
	...
	md := metautils.ExtractIncoming(nCtx).Clone()
	md.Set("newvalue", "something")
	eCtx := md.ToIncoming(nCtx)
	assertRetainsParentContext(t, eCtx)
